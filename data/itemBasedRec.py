...
def test(dataMat, test_ratio, estMethod):
    ...
    for i in range(0,dataMat.shape[1]):
        ...
        total_error += error_u
        total_test_cases += count_u
    
    MAE = total_error / total_test_cases * 1.0
...
def print_most_similar_jokes(dataMat, jokes, queryJoke, k, metric=pearsSim):
	...
	u, s, vt = la.svd(dataMat, full_matrices=False)
	...
	# In information retrieval, a query is compared to documents using vector-space similarity between the query vector and document vectors. 
	# In the lower dim. space, this can be achieved by first mapping the query to lower dim space, and then comparing it to docs in the 
	# lower dim space

	queryVector = dataMat[:,queryJoke]
	...
	print 'Query joke: ', jokes[queryJoke][1], '\n'
	for i in range(1,k+1):
		print 'Matching rank %i: %s' %(i, jokes[simInds[i]][1]), '\n'
